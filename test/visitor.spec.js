...
describe("mongodb visitor", () => {
	...
	it("expression 5.1.1.1.6: A/b eq 1", () => {
		expect(f).to.deep.eql({'A.b': {$eq: 1}})
	})
	
	it("expression  5.1.1.1.6: A eq '1%20%26%202%2B %2F'", () => {
		expect(f).to.deep.eql({'A': {$eq: "1 & 2+ \/"}})
	})
	...
	it("expression 5.1.1.4.1: contains(A, '%7C')", () => {
		expect(f).to.deep.eql({A: /\|/gi});
	})
	
	it("expression 5.1.1.4.1: contains(A, '1%20%26%202') or contains(D, 'EF')", () => {
		expect(f).to.deep.eql({$or: [{A: /1 & 2/gi}, {D: /EF/gi}]});
	})
	
	it("expression 5.1.1.4.2: endswith(A, '%7C')", () => {
		expect(f).to.deep.eql({A: /\|$/gi});
	})
	...
	it("expression 5.1.1.4.3: startswith(A, '%7C')", () => {
		expect(f).to.deep.eql({A: /^\|/gi});
	})
